...
private:
    ...
    string simulateMatch(const string& participant1, const string& participant2) {
        return (participant1 < participant2) ? participant1 : participant2;  // Simulate match based on name comparison
    }
    ...
    string simulateGroupMatches(const vector<string>& group, vector<string>& losers) {
        ...
        for (const auto& member : group) {
            if (member != winner) {
                losers.push_back(member); // Collect losers
            }
        }
    ...
    string generateRandomDate() {
        random_device rd;
        mt19937 gen(rd());
        uniform_int_distribution<> dis_day(1, 31); // Days in the month
        uniform_int_distribution<> dis_hour(0, 23); // Hours
        uniform_int_distribution<> dis_minute(0, 59); // Minutes

        int day = dis_day(gen);
        int month = 12; // Fixed month (e.g., December)
        int year = 2024; // Fixed year
        int hour = dis_hour(gen);
        int minute = dis_minute(gen);

        stringstream date;
        date << setfill('0') << setw(2) << day << "/" << setw(2) << month << "/" << year;
        date << " " << setw(2) << hour << ":" << setw(2) << minute; // Append time in HH:MM format
        return date.str();
    }
...
public:
    ...
    void Grouping(int totalGroups) {
        ...
        shuffle(participants.begin(), participants.end(), gen);  // Shuffle the participants randomly

        groups.clear();  // Clear any existing groups
        int membersPerGroup = participants.size() / totalGroups;  // Calculate number of members per group

        groups.resize(totalGroups);

        for (int i = 0; i < participants.size(); i++) {
            groups[i % totalGroups].push_back(participants[i]);  // Distribute participants into groups
        }
    ...
    // Display the tournament bracket (upper and lower) in 2D format
